...
function removeFirstOccurrences(str, value) {
  ...
  return (
    str.slice(0, valIndex) + str.slice(valIndex + value.length, str.length)
  );
}
...
function isPalindrome(str) {
  let clearStr = str.toLocaleLowerCase();
  clearStr = clearStr.replace(/\W/gi, '');
...
function encodeToRot13(str) {
  ...
  const rotABC = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  return str.replace(/[A-Z]/gi, (letter) => rotABC[normalABC.indexOf(letter)]);
}
